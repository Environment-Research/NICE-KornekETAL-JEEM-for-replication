function cbar = cbarFROMmitigation(MU,P,Tmax)
...
cbar(1,:) = (1-S(1,:)).*Q(1,:)./L(1,:);
c(1,:,:) = bsxfun(@times,cbar(1,:).*((1+D(1,:))./(1-lam(1,:))),q) - bsxfun(@times,cbar(1,:).*D(1,:),d) - bsxfun(@times,cbar(1,:).*((1+D(1,:))./(1-lam(1,:))).*lam(1,:),z);
K(2,:) = S(1,:).*Q(1,:)*10;
...
AD(2,:) = (1-lam(2,:))./(1+D(2,:));
Q(2,:) = AD(2,:).*Y(2,:);
cbar(2,:) = (1-S(2,:)).*Q(2,:)./L(2,:);
c(2,:,:) = max(bsxfun(@times,cbar(2,:).*((1+D(2,:))./(1-lam(2,:))),q) - bsxfun(@times,cbar(2,:).*D(2,:),d) - bsxfun(@times,cbar(2,:).*((1+D(2,:))./(1-lam(2,:))).*lam(2,:),z),tol);
K(3,:) = S(2,:).*Q(2,:)*10;
for t=3:(Tmax-1)
    ...
    AD(t,:) = (1-lam(t,:))./(1+D(t,:));
    Q(t,:) = AD(t,:).*Y(t,:);
    cbar(t,:)= (1-S(t,:)).*Q(t,:)./L(t,:);
    c(t,:,:) = max(bsxfun(@times,cbar(t,:).*((1+D(t,:))./(1-lam(t,:))),q) - bsxfun(@times,cbar(t,:).*D(t,:),d) - bsxfun(@times,cbar(t,:).*((1+D(t,:))./(1-lam(t,:))).*lam(t,:),z),tol);
    K(t+1,:) = S(t,:).*Q(t,:)*10;
end
...
cbar(Tmax,:) = (1-S(Tmax,:)).*Q(Tmax,:)./L(Tmax,:);
c(Tmax,:,:) = max(bsxfun(@times,cbar(Tmax,:).*((1+D(Tmax,:))./(1-lam(Tmax,:))),q) - bsxfun(@times,cbar(Tmax,:).*D(Tmax,:),d) - bsxfun(@times,cbar(Tmax,:).*((1+D(Tmax,:))./(1-lam(Tmax,:))).*lam(Tmax,:),z),tol);