...
loadDATAforNICE
% set
rhov = [0.015]; %[0.015]; %[0 0.01 0.02];
etav = [0.5 1 1.5 2]; %[0 1.01 2 3];
xiv = [0]; %x=1 -> damages are proportional to income; x=0 damages are independent of income; x=-1 ->damages are inverse proportional to income 
omegav = [1];
T7v = [0]; %[0 0.319 0.48];
for i = 1 %1:3
    rho = rhov(i);
...
%OPTIMISATION
    ...
    results = endo;  %results will be stored extra when checked whether converged and, if not converged, optimization is redone with computationally more extensive alternative algorithm
        ...
        switch eta
            case 1 
                results_CASE4a=results; %w. eta=1
            case 1.5 
                results_CASE4b=results; %w. eta=1.5
            case 0.5
                results_CASE4c=results; %w. eta=0.5
            case 2 
                results_CASE4d=results; %w. eta=1
        end
