function c = endoTAXEDcquintilesFROMtax(tax,P,Tmax,tmax)
...
%   PRELIMINARIES
    ...
    L=P.L/1000;
    para=P.para;
    %   The redistributive tax
    fau = P.tau; %marginal rate 
    r = P.regions; %index vector counting the regions
    %   Consumption
    c = zeros(Tmax,5,12);
...
cbar = (1-S(1,:)).*Q(1,:)./L(1,:);
theta = fau*D(1,r).*cbar(r);
c(1,:,:) = bsxfun(@times,cbar.*(1+D(1,:)),q) - bsxfun(@times,cbar.*D(1,:),p*d+(1-p)*q);
c(1,:,r) = bsxfun(@plus,theta,bsxfun(@times,(1-fau*D(1,r)),squeeze(c(1,:,r))));
K(2,:) = S(1,:).*Q(1,:)*10;
...
Q(2,:) = AD(2,:).*Y(2,:);
cbar= (1-S(2,:)).*Q(2,:)./L(2,:);
theta = fau*D(2,r).*cbar(r);
c(2,:,:) = bsxfun(@times,cbar.*(1+D(2,:)),q) - bsxfun(@times,cbar.*D(2,:),p*d+(1-p)*q);
c(2,:,r) = bsxfun(@plus,theta,bsxfun(@times,(1-fau*D(2,r)),squeeze(c(2,:,r))));
K(3,:) = S(2,:).*Q(2,:)*10;
for t=3:(Tmax-1)
    ...
    Q(t,:) = AD(t,:).*Y(t,:);
    cbar= (1-S(t,:)).*Q(t,:)./L(t,:);
    theta = fau*D(t,r).*cbar(r);
    c(t,:,:) = bsxfun(@times,cbar.*(1+D(t,:)),q) - bsxfun(@times,cbar.*D(t,:),p*d+(1-p)*q);
    c(t,:,r) = bsxfun(@plus,theta,bsxfun(@times,(1-fau*D(t,r)),squeeze(c(t,:,r))));
    K(t+1,:) = S(t,:).*Q(t,:)*10;
end
...
theta = fau*D(Tmax,r).*cbar(r);
c(Tmax,:,:) = bsxfun(@times,cbar.*(1+D(Tmax,:)),q) - bsxfun(@times,cbar.*D(Tmax,:),p*d+(1-p)*q);
c(Tmax,:,r) = bsxfun(@plus,theta,bsxfun(@times,(1-fau*D(Tmax,r)),squeeze(c(Tmax,:,r))));
