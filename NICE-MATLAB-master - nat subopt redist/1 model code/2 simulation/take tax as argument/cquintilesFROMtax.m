function c = cquintilesFROMtax(tax,P,Tmax,tmax)
...
mu60a = max(bsxfun(@rdivide,TAX,pb).^(1/(th2-1)),0);
mu60b = min(mu60a,1);
mu = ones(60,12);
mu(1:tmax,:) = mu60b(1:tmax,:);
%abatement cost
lam = th1.*mu.^th2;
...
c(2,:,:) = max(bsxfun(@times,cbar(2,:).*(1+D(2,:)),q) - bsxfun(@times,cbar(2,:).*D(2,:),p*d+(1-p)*q),tol);
K(3,:) = S(2,:).*Q(2,:)*10;
for t=3:(Tmax-1)
    ...
    c(t,:,:) = max(bsxfun(@times,cbar(t,:).*(1+D(t,:)),q) - bsxfun(@times,cbar(t,:).*D(t,:),p*d+(1-p)*q),tol);
    K(t+1,:) = S(t,:).*Q(t,:)*10;
end
...
c(Tmax,:,:) = max(bsxfun(@times,cbar(Tmax,:).*(1+D(Tmax,:)),q) - bsxfun(@times,cbar(Tmax,:).*D(Tmax,:),p*d+(1-p)*q),tol);
