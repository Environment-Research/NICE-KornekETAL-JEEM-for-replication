function c = cquintilesNOdamageB(tax,P,Tmax,tmax)
...
Q(2,:) = AD(2,:).*Y(2,:);
cbar= (1-S(2,:)).*Q(2,:)./L(2,:);
c(2,:,:) = max(bsxfun(@times,cbar.*(1+D(2,:)),q),tol); % - bsxfun(@times,cbar.*D(2,:),p*d+(1-p)*q),tol);
K(3,:) = S(2,:).*Q(2,:)*10;
for t=3:(Tmax-1)
    ...
    Q(t,:) = AD(t,:).*Y(t,:);
    cbar= (1-S(t,:)).*Q(t,:)./L(t,:);
    c(t,:,:) = max(bsxfun(@times,cbar.*(1+D(t,:)),q),tol); % - bsxfun(@times,cbar.*D(t,:),p*d+(1-p)*q),tol);
    K(t+1,:) = S(t,:).*Q(t,:)*10;
end
...
c(Tmax,:,:) = max(bsxfun(@times,cbar.*(1+D(Tmax,:)),q),tol); % - bsxfun(@times,cbar.*D(Tmax,:),p*d+(1-p)*q),tol);
