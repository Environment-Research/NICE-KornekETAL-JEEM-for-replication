function c = ODAcquintilesFROMtax(tax,P,Tmax,tmax)
...
cake = L(1,donors)*squeeze(sum(ODAtau*c(1,:,donors),2))/5;
c(1,:,donors) = (1-ODAtau)*c(1,:,donors);
mouths = L(1,r)*squeeze(sum(c(1,:,r),2))/5;
c(1,:,r) = (1-pi)*(1+cake/mouths)*c(1,:,r) + pi*(c(1,:,r) + cake/sum(L(1,r),2));
K(2,:) = S(1,:).*Q(1,:)*10;
...
cake = L(2,donors)*squeeze(sum(ODAtau*c(2,:,donors),2))/5;
c(2,:,donors) = (1-ODAtau)*c(2,:,donors);
mouths = L(2,r)*squeeze(sum(c(2,:,r),2))/5;
c(2,:,r) = (1-pi)*(1+cake/mouths)*c(2,:,r) + pi*(c(2,:,r) + cake/sum(L(2,r),2));
K(3,:) = S(2,:).*Q(2,:)*10;
for t=3:(Tmax-1)
    ...
    cake = L(t,donors)*squeeze(sum(ODAtau*c(t,:,donors),2))/5;
    c(t,:,donors) = (1-ODAtau)*c(t,:,donors);
    mouths = L(t,r)*squeeze(sum(c(t,:,r),2))/5;
    c(t,:,r) = (1-pi)*(1+cake/mouths)*c(t,:,r) + pi*(c(t,:,r) + cake/sum(L(t,r),2));
    K(t+1,:) = S(t,:).*Q(t,:)*10;
end
