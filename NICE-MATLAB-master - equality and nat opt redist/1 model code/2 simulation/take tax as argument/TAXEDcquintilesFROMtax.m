function c = TAXEDcquintilesFROMtax(tax,P,Tmax,tmax)
...
c(1,:,r) = bsxfun(@plus,theta,(1-tau)*squeeze(c(1,:,r)));
K(2,:) = S(1,:).*Q(1,:)*10;
...
c(2,:,r) = bsxfun(@plus,theta,(1-tau)*squeeze(c(2,:,r)));
K(3,:) = S(2,:).*Q(2,:)*10;
for t=3:(Tmax-1)
    ...
    c(t,:,r) = bsxfun(@plus,theta,(1-tau)*squeeze(c(t,:,r)));
    K(t+1,:) = S(t,:).*Q(t,:)*10;
end
...
c(Tmax,:,r) = bsxfun(@plus,theta,(1-tau)*squeeze(c(Tmax,:,r)));
