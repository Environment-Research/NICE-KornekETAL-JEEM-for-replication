...
%   PRELIMINARIES
    ...
    % weight in national social welfare function
    omegaopt = P.omegaopt; 
    eta = P.eta;
    weight = bsxfun(@rdivide, (omegaopt).^(1/eta),(sum((omegaopt).^(1/eta),1))); 
    %   Capital
    K = zeros(60,12);
    sigma=P.sigma;
    th1 = P.th1;
...
cbar = (1-S(1,:)).*Q(1,:)./L(1,:); 
c(1,:,:) = bsxfun(@times,weight,cbar*5);
K(2,:) = S(1,:).*Q(1,:)*10;
...
Q(2,:) = AD(2,:).*Y(2,:);
cbar= (1-S(2,:)).*Q(2,:)./L(2,:);
c(2,:,:) = max(bsxfun(@times,weight,cbar*5),tol);
K(3,:) = S(2,:).*Q(2,:)*10;
for t=3:(Tmax-1)
    ...
    Q(t,:) = AD(t,:).*Y(t,:);
    cbar= (1-S(t,:)).*Q(t,:)./L(t,:);
    c(t,:,:) = max(bsxfun(@times,weight,cbar*5),tol); 
    K(t+1,:) = S(t,:).*Q(t,:)*10;
end
